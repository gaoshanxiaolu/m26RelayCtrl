...
#define UDP_TIMER_PERIOD     300
#define MOTOR_TIMER_ID         TIMER_ID_USER_START+1
#define MOTOR_TIMER_PERIOD     3000
...
static u8  m_SrvADDR[SRVADDR_BUFFER_LEN] = "106.14.143.71";
static u8  m_ipaddress[5];  //only save the number of server ip, remove the comma
static u32 m_SrvPort = 9002;
...
static void Motor_Timer(u32 timerId, void* param);
/*****************************************************************
* other subroutines
******************************************************************/
...
void proc_main_task(s32 taskId)
{
   // s32 ret;
    ...
    Ql_Timer_Register(MOTOR_TIMER_ID, Motor_Timer, NULL);
    Ql_Timer_Start(MOTOR_TIMER_ID, MOTOR_TIMER_PERIOD, TRUE);

     // Specify a GPIO pin
    Enum_PinName  gpioLight = PINNAME_NETLIGHT;
    Enum_PinName  gpioMotorClockWise = PINNAME_RXD_AUX;
    Enum_PinName  gpioMotorAntiClockWise = PINNAME_TXD_AUX;

    // Define the initial level for GPIO pin
    Enum_PinLevel gpioLvHigh = PINLEVEL_HIGH;
    Enum_PinLevel gpioLvLow = PINLEVEL_LOW;
    
    // Initialize the GPIO pin (output high level, pull up)
    Ql_GPIO_Init(gpioLight, PINDIRECTION_OUT, gpioLvHigh, PINPULLSEL_PULLUP);
    Ql_GPIO_Init(gpioMotorClockWise, PINDIRECTION_OUT, gpioLvHigh, PINPULLSEL_PULLUP);
    Ql_GPIO_Init(gpioMotorAntiClockWise, PINDIRECTION_OUT, gpioLvLow, PINPULLSEL_PULLUP);
    APP_DEBUG("<-- Initialize GPIO pin (PINNAME_STATUS): output, high level, pull up -->\r\n");

    // Get the direction of GPIO
    APP_DEBUG("<-- Get the GPIO direction: %d -->\r\n", Ql_GPIO_GetDirection(gpioLight));

    // Get the level value of GPIO
    APP_DEBUG("<-- Get the GPIO level value: %d -->\r\n\r\n", Ql_GPIO_GetLevel(gpioLight));

    // Set the GPIO level to low after 500ms.
    APP_DEBUG("<-- Set the GPIO Light level to low after 500ms -->\r\n");
    Ql_Sleep(500);
    Ql_GPIO_SetLevel(gpioLight, PINLEVEL_LOW);
    APP_DEBUG("<-- Get the GPIO Light level value: %d -->\r\n\r\n", Ql_GPIO_GetLevel(gpioLight));

    // Set the GPIO level to high after 500ms.
    APP_DEBUG("<-- Set the GPIO Light level to high after 500ms -->\r\n");
    Ql_Sleep(500);
    Ql_GPIO_SetLevel(gpioLight, PINLEVEL_HIGH);
    APP_DEBUG("<-- Get the GPIO Light level value: %d -->\r\n", Ql_GPIO_GetLevel(gpioLight));
...
static void Motor_Timer(u32 timerId, void* param)
{
   ...
   static s32 Count=0;
    ...
    if (MOTOR_TIMER_ID == timerId)
    {
        //APP_DEBUG("<--...........m_udp_state=%d..................-->\r\n",m_udp_state);
        if(Count%2==0) 
            {
            	 Ql_GPIO_SetLevel(gpioMotorAntiClockWise, PINLEVEL_LOW);
		 Ql_Sleep(500);
            	 Ql_GPIO_SetLevel(gpioMotorClockWise, PINLEVEL_HIGH);
            	 APP_DEBUG("<-- Clockwise Motor ---> \r\n");
            }
            else if(Count%2==1) 
            {
              Ql_GPIO_SetLevel(gpioMotorClockWise, PINLEVEL_LOW);
		 Ql_Sleep(500);
            	 Ql_GPIO_SetLevel(gpioMotorAntiClockWise,PINLEVEL_HIGH );
		 
            	
            	 APP_DEBUG("<-- AntiClockwise Motor --->\r\n ");
            }
            Count++;
    	}
}
static void Callback_Timer(u32 timerId, void* param)
{
    char strTest[]={0x8e,0x88,0x30,0x5c,0x57,0xa2,0x00,0x00,0xbb,0x00,0x07,0x00,0x00,0x00,0x00,0x00,0xad,0x8f};
    static s32 Count=0;
    if (UDP_TIMER_ID == timerId)
    {
        ...
        switch (m_udp_state)
        {
			...
			break;
                ...
                {
                	m_pCurrentPos = m_send_buf;
                	Ql_memcpy(m_pCurrentPos, strTest, sizeof(strTest));
                	m_remain_len = sizeof(strTest);
                	APP_DEBUG("<--Send data, data len=%d-->\r\n",m_remain_len);
                }
