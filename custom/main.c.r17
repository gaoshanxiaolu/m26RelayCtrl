...
#define CTRL_TIMER_ID        TIMER_ID_USER_START + 1   //timeout

#define TCP_TIMER_PERIOD     1000
#define CTRL_TIMER_PERIOD    50
...
static u8  m_SrvADDR[SRVADDR_BUFFER_LEN] = "47.99.120.255";
static u32 m_SrvPort = 6666;
...
* Gpio Param
******************************************************************/
Enum_PinName m_gpioMotorP = PINNAME_RXD_AUX;
Enum_PinName m_gpioMotorN = PINNAME_TXD_AUX;
Enum_PinName m_gpioLight  = PINNAME_NETLIGHT;
...
static void Callback_Timer50ms(u32 timerId, void* param);
...
void proc_main_task(s32 taskId)
{
    ...
    APP_DEBUG("<--Start Massage App procedure-->\r\n");
    ...
    Ql_Timer_Register(TCP_TIMER_ID, Callback_Timer, NULL);
	  Ql_Timer_Register(CTRL_TIMER_ID, Callback_Timer50ms, NULL);
    Ql_Timer_Start(TCP_TIMER_ID, TCP_TIMER_PERIOD, TRUE);
    Ql_Timer_Start(CTRL_TIMER_ID, CTRL_TIMER_PERIOD, TRUE);
    ...
    while(TRUE)
    {
        ...
        case MSG_ID_RIL_READY:
			...
			if(ret == RIL_AT_SUCCESS)
			{
				...
				CalcCommunicationID(m_id, &strImei[5u], 0x01u);       /* auto chare station */
				APP_DEBUG("<-- calculate ID is :%x %x %x %x %x -->\r\n", m_id[0], m_id[1],m_id[2],m_id[3],m_id[4]);
				GenerateHeartBeatData(m_id, 0x01u);
			}
            break;
#endif
        default:
...
static void Callback_Timer(u32 timerId, void* param)
{
    ...
    {
        APP_DEBUG("Light %d, Positive motor %d, Negative motor %d\r\n", Ql_GPIO_GetLevel(m_gpioLight), Ql_GPIO_GetLevel(m_gpioMotorP), Ql_GPIO_GetLevel(m_gpioMotorN));
    		Timer10sCnt++;
    		if((Timer10sCnt >= 10u) && (m_tcp_state == STATE_SOC_SEND))
    		{
        		dataToServer = GetHeartBeatBuffPtr();
    				TransferGprsSendBuff(dataToServer, 12u);
    	      dataToServer = GetGprsSendBuffPtr();
    	      tmplen = GetGprsSendBuffLength();
    				APP_DEBUG("<--Heart Beat send buff is %d %d %d %d %d %d %d %d %d %d %d %d %d -->\r\n",dataToServer[0],dataToServer[1],dataToServer[2],dataToServer[3],dataToServer[4],dataToServer[5],dataToServer[6],dataToServer[7], dataToServer[8], dataToServer[9], dataToServer[10], dataToServer[11], dataToServer[12]);
    			
          	m_pCurrentPos = m_send_buf;
    			  prelen = m_remain_len;
    			  MyHexCopy(m_pCurrentPos + m_remain_len, dataToServer, tmplen);
            m_remain_len = prelen + tmplen;
    			  Timer10sCnt = 0u;
    		}
...
void callback_socket_read(s32 socketId, s32 errCode, void* customParam )
{
    ...
    do
    {
        ...
        {
            if ((0 == m_recv_buf[7u]) && (0xAB == m_recv_buf[8u]))
            {
                //if ((MASSAGE_START_CMD == m_recv_buf[14u]) && (m_recv_buf[20u]== CalcCheckSum(&m_recv_buf[11u],9)))   // start cmd
                if ((MASSAGE_START_CMD == m_recv_buf[14u]))
                {
                    APP_DEBUG("<-- receive start cmd -->\r\n");
                    SetMassageRunTime(60 * m_recv_buf[19]);
                    SetMotorPositiveRunTime(m_recv_buf[16]);
                    SetMotorReverseRunTime(m_recv_buf[17]);
                    if(m_recv_buf[18] != 0u)
                    {
                      SetLedOnCmd(TRUE);
                    }
                    ...
                    SetMassageStartCmd(TRUE);
                    SetMassageStopCmd(FALSE);
                    GenerateDeviceStartWorkAck(m_id, 1u, 1u);
					
										dataToServer = GetProtocolBuffPtr();
										tmplen = GetProtocolBuffLength();
										TransferGprsSendBuff(dataToServer, tmplen);
										dataToServer = GetGprsSendBuffPtr();
										tmplen = GetGprsSendBuffLength();
                ...
                //else if ((MASSAGE_STOP_CMD == m_recv_buf[14u]) && (m_recv_buf[16u]== CalcCheckSum(&m_recv_buf[11u],5)))   // stop cmd
                else if ((MASSAGE_STOP_CMD == m_recv_buf[14u]))   
                {
                    APP_DEBUG("<-- receive stop cmd -->\r\n");  
                    SetMassageStartCmd(FALSE);
                    SetMassageStopCmd(TRUE);
                    GenerateDeviceStopWorkAck(m_id, 1u, 1u);
                ...
                {
                   //APP_DEBUG("<-- receive illegal cmd -->\r\n"); 
                }
            }
			else if((0 == m_recv_buf[7u]) && (0xBB == m_recv_buf[8u]))
			{
				ClearNoServerHeartBeatAckCnt();
				APP_DEBUG("<-- Receive heart beat ack, ");  
				
				if(m_recv_buf[10] == 1u)
				{
					APP_DEBUG("<-- sever normal -->\r\n");  
				}
				else
				{
					APP_DEBUG("<-- sever abnormal -->\r\n"); 
				}
			}
